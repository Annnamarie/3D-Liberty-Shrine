...
{
	// Adjust zoom level or speed based on scroll
	g_pCamera->Zoom -= (float)yOffset;
	if (g_pCamera->Zoom < 1.0f) g_pCamera->Zoom = 1.0f;
	if (g_pCamera->Zoom > 45.0f) g_pCamera->Zoom = 45.0f;
}
...
void ViewManager::ProcessKeyboardEvents()
{
	...
	{
		//this is to move forward -> W
		g_pCamera->ProcessKeyboard(FORWARD, gDeltaTime);
	}
	if (glfwGetKey(m_pWindow, GLFW_KEY_S) == GLFW_PRESS)
	{
		//this is to move backward -> S
	...
	// Q and E keys to control upward
...
void ViewManager::PrepareSceneView()
{
	...
	//projection matrix based on orthographic setting
